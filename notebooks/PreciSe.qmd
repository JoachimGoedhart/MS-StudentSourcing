...
The [SuperPlotsOfData app](https://huygens.science.uva.nl/SuperPlotsOfData/) plots the data and calculates the statistics. We also do the calculations here. First we generate a dataframe that summarizes all individual measurements per independent measurement (defined by the column 'Replicate'): 

```{r}
df_summary <- df_results %>% group_by(Replicate) %>% summarize(n=n(), Percentage=mean(S_phase))
df_summary$Condition <- "all"
df_summary
```
...
df_S <- df_summary %>% ungroup() %>% summarise(n=n(),
  ...
  mutate(sem = sd / sqrt(n - 1),
         mean_CI_lo = avg + qt((1-0.95)/2, n - 1) * sem,
         mean_CI_hi = avg - qt((1-0.95)/2, n - 1) * sem)
```
