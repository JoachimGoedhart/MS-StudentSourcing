...
Let's now generate a superplot for the individual replicates, repeating what can be done in the app with the data in `results_S-phase.csv``:
...
p <- p + geom_quasirandom(
                           varwidth = TRUE, cex=2, alpha=0.8, groupOnX=TRUE)
...
```{r}
p <- p + geom_flat_violin(color=NA, scale = "width", width=0.7,position = position_nudge(x = .22, y = 0), trim=FALSE, alpha = 0.8)

p <-  p + stat_summary(fun = mean, geom = "point", shape=21, stroke = .3, size=5, color="black", alpha=0.8)
